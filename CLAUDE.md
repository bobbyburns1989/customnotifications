...
| **Model** | Free (all features unlocked). RevenueCat freemium planned for v1.1. |
| **Website** | customnotify.us |
...
- [x] RevenueCat service (purchases_flutter wrapper, disabled for v1.0 - placeholder API keys)
- [x] Custom paywall screen (branded, feature comparison - unreachable in v1.0)
- [x] Premium gate (NotificationService bypasses limits - hardcoded to unlocked for v1.0)
- [x] Premium schedule type validation at service layer (defense-in-depth)
- [x] Templates system (50 built-in templates, JSON asset, gallery screen with category filter chips)
...
- [x] UI polish pass: clean error messages, centralized strings/sizes, badge contrast, accessibility labels
...
### Post-launch (v1.1+)
- [ ] Enable RevenueCat freemium (replace placeholder API keys, re-enable init, restore stream provider)
...
- User-facing strings go in `AppStrings` - never hardcode inline (error messages, empty states, hints, labels)
- Spacing values go in `AppSizes` - avoid raw pixel literals for gaps and padding
- Add `tooltip` to FABs and icon-only buttons for screen reader accessibility
...
- **Never use Firebase or any cloud database** - no network calls in v1.0 (RevenueCat disabled)
...
## Freemium Rules (v1.1 - disabled for v1.0 launch)

All features are unlocked in v1.0. The premium provider hardcodes `true` in
`lib/presentation/providers/premium_provider.dart`. To re-enable the freemium
gate: configure RevenueCat API keys, uncomment the init call in `main.dart`,
and restore the stream-based `premiumStatusProvider`.
...
- **Filter chips**: Pill-shaped, gold fill when selected, outline when not; used on Templates screen for category filtering
...
| Template list + filter providers | `lib/presentation/providers/template_provider.dart` |
